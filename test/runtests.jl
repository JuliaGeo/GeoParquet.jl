...
@testset "GeoParquet.jl" begin
    ...
    @testset "Writing" begin
        ...
        # fn = "data/example.parquet"
        # @time df = GeoParquet.read(fn)
        # @time GeoParquet.write("data/example_copy.parquet", df, (:geometry,))
    end

    @testset "Parquet2 FilePath" begin
        df = DataFrame(a=1)
        Parquet2.writefile("test.parquet", df)
        mv("test.parquet", "test2.parquet")
        ds = Parquet2.Dataset("test2.parquet")
        DataFrame(ds)
